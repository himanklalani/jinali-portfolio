...
function buildItems(pool: ImageItem[], seg: number): ItemDef[] {
  ...
  if (pool.length === 0) {
    return coords.map(c => ({ ...c, src: '', alt: '', originalIndex: 0 }));
  }
  ...
  const normalizedImages = pool.map((image, idx) => {
    if (typeof image === 'string') {
      return { src: image, alt: '', originalIndex: idx };
    }
    return { src: image.src || '', alt: image.alt || '', originalIndex: idx };
  });
...
export default function DomeGallery({
  ...
  const onTileClick = useCallback(
    (e: React.MouseEvent<HTMLDivElement>, originalIndex: number) => {
  ...
  const onTilePointerUp = useCallback(
    (e: React.PointerEvent<HTMLDivElement>, originalIndex: number) => {
            ...
            {items.map((it, i) => (
              ...
              >
                <div
                  ...
                  onClick={(e) => onTileClick(e, it.originalIndex)}
                  onPointerUp={(e) => onTilePointerUp(e, it.originalIndex)}
                >
